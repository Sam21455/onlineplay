    ...
    public void onMessage(String message, Session session) throws IOException {
        ...
        switch (updateType) {
            case "colorsGrid":
                ...
                // Envoyer la liste des coups joués au client WebSocket
                JsonArrayBuilder moveHistoryArrayBuilder = Json.createArrayBuilder();
                for (Move move : moveHistory) {
                    JsonObject moveJson = Json.createObjectBuilder()
                            ...
                            .add("color", (move.getUserId() == game.getFirstPlayer().getId()) ? "red" : "green")
                            .build();
                    moveHistoryArrayBuilder.add(moveJson);
                }

                JsonObject moveHistoryMessage = Json.createObjectBuilder()
                        .add("update", "moveHistory")
                        .add("moves", moveHistoryArrayBuilder.build())
                        .build();
                ...
                JsonObject moveJson = 
                        Json.createObjectBuilder()
                                ...
                                .add("first_player_user_id", game.getFirstPlayer().getId())
                                .add("second_player_user_id", game.getSecondPlayer().getId())
                                .add("number", coordinatesPlayed.getX()) // ligne
                                .add("move", coordinatesPlayed.getY()) // colonne
                                .build();
                ...
                for (Session webSocketSession : GameWebSocket.getSessionsRelatedToGameId(game.getId())) {
                    if (webSocketSession != null && webSocketSession.isOpen()) {
                        try {
                            ...
                            // envoie les moves (= les coups joués) à react
                            webSocketSession
                                    .getBasicRemote().
                                    sendText(
                                        moveJson.toString()
